...
public class ExternalTaskExpressionSupportTest extends PluggableProcessEngineTestCase {
  ...
  @Test
  @Deployment(resources = "org/camunda/bpm/engine/test/api/externaltask/ExternalTaskSupportTest.expressionEmpty.bpmn20.xml")
  public void testExternalTaskSupportUsingEmptyVariable() {

    HashMap<String, Object> globalVariables = new HashMap<String, Object>();
    globalVariables.put("testVariable", "");
    try {
      runtimeService.startProcessInstanceByKey("oneExternalTaskProcessEmpty", globalVariables);
      Assert.fail("starting process did not throw an exception although topic name is invalid");
    } catch (Exception e) {
      Throwable cause = e;
      while (cause.getCause() != null) {
        cause = cause.getCause();
      }
      Assert.assertTrue(e.getMessage().indexOf("valid topicname") != -1);
    }
